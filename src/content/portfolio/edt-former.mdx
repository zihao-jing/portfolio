---
title: 'EDT-Former: Adaptive Connectors Let Frozen LLMs Handle Multimodal Inputs of Varying Scale'
date: '2026-01-21'
...
**Situation:** Connectors that link structure encoders to LLMs use a fixed number of tokens regardless of input complexity — losing critical structural detail for large inputs while wasting capacity for simple ones, and requiring expensive LLM fine-tuning to compensate. **Task:** Design a connector that automatically scales its token budget to input complexity without touching the LLM backbone. **Action:** Built an entropy-guided dynamic connector that identifies information-dense boundaries in each input's structure and allocates more tokens where complexity is high, keeping both encoder and LLM fully frozen during training. **Result:** Achieves top-1 across property prediction, reasoning, and generation benchmarks, running 3.5× faster with half the GPU memory of standard fine-tuning — making scalable structure-to-language alignment practical.
...
A lightweight predictor, trained to model sequential structure, produces high entropy at positions where the next element is hard to predict — boundaries, transitions, and structural junctions. These entropy peaks become natural split points, dividing the input into segments. Each segment is pooled into a single token, so more complex inputs automatically produce more tokens.
...
EDT-Former demonstrates that strong structure-to-language alignment does not require fine-tuning the LLM — a connector that adapts token budgets to multimodal input complexity outperforms systems with full backbone training. The adaptive design is backbone-agnostic and generalizes to any structured modality with a sequence-ordered representation.
