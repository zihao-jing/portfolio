...
void main() {
  ...
  if (uAutoCenterRepulsion > 0.0) {
    ...
    float falloff = exp(-mouseDist / uRepulsionRadius);
    vec2 repulsion = normalize(uv - mousePosUV) * (uRepulsionStrength / (mouseDist + 0.1)) * falloff;
...
export function GalaxyBackground({
  ...
  useEffect(() => {
    ...
    function update(t: number) {
      ...
      smoothMouse.current.x += (targetMouse.current.x - smoothMouse.current.x) * lerpFactor;
      smoothMouse.current.y += (targetMouse.current.y - smoothMouse.current.y) * lerpFactor;
      smoothActive.current  += (targetActive.current  - smoothActive.current)  * lerpFactor;
    ...
    function onMouseMove(e: MouseEvent) {
      ...
      // Track anywhere inside the hero bounds
      if (x >= 0 && x <= 1 && y >= 0 && y <= 1) {
  ...
  return (
    <div
      ...
      aria-hidden="true"
      style={{ overflow: "hidden", mixBlendMode: blendMode }}
    />
  );
}
